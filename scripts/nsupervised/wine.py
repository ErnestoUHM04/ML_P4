...
# For this one, we got the best k result is 4, even though we are actually working with 1-10 (k=10)
# So we are trying both
k = 10
...
#y = wine[["quality"]] # we aint using it here
...
plt.tight_layout()
#plt.savefig("results/wine_nsupervised_k4_regressors_comparison.png") # Save the figure
plt.savefig("results/wine_nsupervised_k10_regressors_comparison.png") # Save the figure
plt.show()

# Command to run the script and save the terminal output
#python scripts/nsupervised/wine.py | tee results/wine_nsupervised_k2_terminal_output.txt
#python scripts/nsupervised/wine.py | tee results/wine_nsupervised_k10_terminal_output.txt